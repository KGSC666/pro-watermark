...
export const EditorLayout: React.FC<EditorLayoutProps> = ({ canvas, controls, onExport, onAdd, onDropFiles }) => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") setShowLangMenu(false); };
      ...
      <main className="flex-1 relative flex items-center justify-center min-w-0 transition-all duration-500">
        ...
        {isDesktop && (
          <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-2 p-1.5 bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl z-20">
             <button onClick={onAdd} title="Add Image" className="p-2.5 hover:bg-white/10 rounded-xl transition-all active:scale-95 text-white/60 hover:text-white">
                <Plus size={20} />
             </button>
             ...
             <button onClick={onExport} className="flex items-center gap-2 px-5 py-2 bg-white text-black rounded-xl font-bold text-sm hover:bg-neutral-200 transition-all active:scale-95">
        ...
        {!isDesktop && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 p-1.5 bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl z-40">
            <button onClick={onAdd} title={t('add_image')} className="p-2.5 hover:bg-white/10 rounded-xl transition-all active:scale-95 text-white/60 hover:text-white">
              <Plus size={20} />
            </button>
            <div className="w-[1px] h-5 bg-white/10" />
            <button onClick={onExport} className="flex items-center gap-2 px-4 py-2 bg-white text-black rounded-xl font-bold text-sm active:scale-95 transition-transform">
              <Download size={16} /> {t('export')}
            </button>
          </div>
        )}
      ...
      {isDesktop ? (
        <aside
          className={`h-full bg-neutral-900/40 backdrop-blur-3xl border-l border-white/5 flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.5)] overflow-hidden z-10 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
            showSidebar ? 'w-[320px] opacity-100 translate-x-0' : 'w-0 opacity-0 translate-x-20'
          }`}
        >
          <div className="w-[320px] p-8 flex flex-col gap-8 h-full">
            <div className="flex items-center justify-between shrink-0">
              <h2 className="text-xl font-bold tracking-tight text-white/90">{t('inspector')}</h2>
              <button onClick={() => setShowSidebar(false)} className="text-neutral-600 hover:text-white transition-colors">
