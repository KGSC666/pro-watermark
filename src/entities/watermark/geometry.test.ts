...
describe('relToPoint / pointToRel', () => {
    ...
    it('keeps the same relative spot across different image sizes (per-type persistence)', () => {
        ...
        expect(pointToRel(small, { centerX: 400, centerY: 240, width: 800, height: 480 })).toEqual(
            rel,
        );
        expect(pointToRel(large, { centerX: 600, centerY: 360, width: 1200, height: 720 })).toEqual(
            rel,
        );
    });
    ...
    it('guards against a zero-sized image', () => {
        expect(
            pointToRel({ left: 0, top: 0 }, { centerX: 0, centerY: 0, width: 0, height: 0 }),
        ).toEqual({ x: 0.5, y: 0.5 });
    });
});
